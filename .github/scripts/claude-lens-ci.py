#!/usr/bin/env python3
"""
claude-lens CI wrapper — same logic as ~/.claude/scripts/claude-lens.py
but skills_dir defaults to the project's own skills/ folder.
Run from repo root:
  python3 .github/scripts/claude-lens-ci.py --since 7 --print
"""
import argparse
import json
import re
...
def fetch_youtube_videos(since: datetime, max_videos: int = 20) -> list[dict]:
    if not HAS_YTDLP:
        print("  [youtube] yt-dlp not installed, skipping", file=sys.stderr)
        return []
    results = []
    ...
    for entry in entries:
        ...
        if HAS_YT_TRANSCRIPT and video_id:
            try:
                transcript = YouTubeTranscriptApi.get_transcript(video_id, languages=["en", "sv", "en-US"])
...
def audit_skills(skills_dir: Path) -> list[str]:
    existing_text = ""
    for d in [skills_dir, Path.cwd() / "skills"]:
        if d.exists():
...
def main():
    ...
    parser.add_argument("--no-skills-audit", action="store_true")
    parser.add_argument("--print", dest="print_only", action="store_true")
    args = parser.parse_args()

    cutoff = since_dt(args.since)
    # In CI, check project skills dir; fallback to home
    ...
    out_dir = args.output or Path("docs/claude-updates")
    out_dir.mkdir(parents=True, exist_ok=True)
    week = datetime.now().strftime("%Y-W%V")
    out_file = out_dir / f"{week}.md"
