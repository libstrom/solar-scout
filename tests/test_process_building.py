...
def _make_process_building_patches(analyze_return, has_extra_solar=False, img_bytes=b"fake_image"):
    ...
    _analyze_building, _analyze_building_opus, _prefilter_building, _fetch_street_view, and
    ...
    that focus on _analyze_building behaviour are not affected by the Haiku pre-filter.
    """
    ...
    # When first pass is UNSURE, _analyze_building_opus is called; mirror the first-pass
    # result so UNSURE tests still yield a needs_review lead.
    is_unsure = analyze_return[2]
    opus_return = (True, False, True, analyze_return[3]) if is_unsure else analyze_return
    stack.enter_context(patch("scanner._analyze_building_opus", return_value=opus_return))
